...
def ThomasBlockSolver(A : sp.csc_matrix, b : np.ndarray, block_size : int) -> np.ndarray:
    ...
    # Build block lists with fixed ordering per block: [L_i, D_i, U_i]
    A_blocks = []
    B_blocks = []
    zero_blk = sp.csc_matrix(np.zeros((block_size, block_size)), dtype=np.complex128)
    ...
    for i in range(n_blocks):
        ...
        if i == 0:
            ...
            L = GetBlockMatrix(A, i*block_size, (i+1)*block_size - 1,
                               (i-1)*block_size, i*block_size - 1)

        # Diagonal block (D_i)
        D = GetBlockMatrix(A, i*block_size, (i+1)*block_size - 1,
        ...
        if i == n_blocks - 1:
            ...
            U = GetBlockMatrix(A, i*block_size, (i+1)*block_size - 1,
                               (i+1)*block_size, (i+2)*block_size - 1)

        A_blocks.extend([L, D, U])
...
#
        ...
        # previous block indices
        idx_prev = 3*(i-1)
        idx_curr = 3*i

        L_curr = A_blocks[idx_curr + 0]
        D_curr = A_blocks[idx_curr + 1]
        U_prev = A_blocks[idx_prev + 2]
        D_prev = A_blocks[idx_prev + 1]
        B_prev = B_blocks[i-1]
        B_curr = B_blocks[i]

        # Solve D_prev * X = [U_prev | B_prev]
        AugMat = AugmentedMatrix(U_prev.toarray(), B_prev)
        Dinv = np.linalg.inv(D_prev.toarray())
        X = Dinv @ AugMat

        # Update current D and B: [D_curr | B_curr] -= L_curr * X
        AugMat = AugmentedMatrix(D_curr.toarray(), B_curr)
        AugMatNew = AugMat - L_curr.toarray() @ X
        D_new, B_new = UndoAugmentedMatrix(AugMatNew)
        A_blocks[idx_curr + 1] = D_new
        B_blocks[i] = B_new

    # Zero out L_curr and set previous D to identity, previous U to solved U_new
    #A_blocks[idx_curr + 0] = sp.csc_matrix(np.zeros((block_size, block_size)), dtype=np.complex128)
    #A_blocks[idx_prev + 1] = sp.csc_matrix(np.eye(block_size), dtype=np.complex128)
    #U_new, B_new = UndoAugmentedMatrix(X)
    #A_blocks[idx_prev + 2] = U_new
    #B_blocks[i-1] = B_new

    # Optional: reconstruct A' for sanity
    #Amod = np.zeros(A.shape, dtype=np.complex128)
    #for i in range(n_blocks):
    #    idx = 3*i
    #    Amod[i*block_size:(i+1)*block_size, (i-1)*block_size:i*block_size] = A_blocks[idx + 0].toarray()
    #    Amod[i*block_size:(i+1)*block_size, i*block_size:(i+1)*block_size] = A_blocks[idx + 1].toarray()
    #    Amod[i*block_size:(i+1)*block_size, (i+1)*block_size:(i+2)*block_size] = A_blocks[idx + 2].toarray()

    #print(sp.csc_matrix(Amod))
    # Back substitution
    X_blocks = []
    for i in range(n_blocks - 1, -1, -1):
        idx = 3*i
        D_curr = A_blocks[idx + 1]
        B_curr = B_blocks[i]

        if i == n_blocks - 1:
            #Last block
            Dinv = np.linalg.inv(D_curr.toarray())
            X_curr = Dinv @ B_curr
            X_blocks.insert(0, X_curr)
        else:
            U_curr = A_blocks[idx + 2]
            X_next = X_blocks[0]
...
if __name__ == '__main__':
    ...
    for i in range(nblk):
        ...
        Ad[i * bs:(i + 1) * bs, i * bs:(i + 1) * bs] = np.eye(bs) * (5.0) + 0.1 * (np.random.randn(bs, bs))
        if i < nblk - 1:
